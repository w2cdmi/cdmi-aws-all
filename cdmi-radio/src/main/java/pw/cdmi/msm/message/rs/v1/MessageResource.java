...
import pw.cdmi.msm.message.model.entity.MessageForUserEntry;
...
public class MessageResource {
    ...
    public JSONObject getMessageForUser(@QueryParam("id") String id) {
        ...
        if(notifyUserMessage.getMessageStatus()==MessageStatus.UNREAD){
    ...
    public JSONObject findAllMessage(@QueryParam("pageNo") int pageNo, @QueryParam("pageSize") int pageSize) {
        ...
        JSONObject json = new JSONObject();
        List<Message> list = messageService.findAllMessage(pageNo, pageSize);
        json.put("datas", JSONArray.fromObject(list));
        int count = messageService.findMessageCount();
    ...
    private JSONObject findMessageByType(MessageType type, int pageNo, int pageSize) {
        JSONObject json = new JSONObject();
        List<Message> productMessages = messageService.findMessageByType(type, pageNo, pageSize);
        json.put("datas", JSONArray.fromObject(productMessages));
        int count = messageService.findMessageCountByType(type);
        int totalPages = count % pageSize > 0 ? (count / pageSize) + 1 : count / pageSize;
        json.put("count", count);
        json.put("totalPages", totalPages);
        return json;
    ...
    public JSONObject findMessageByUserId(@QueryParam("userId") String userId, @QueryParam("pageNo") int pageNo,
        ...
        List<NotifyUserMessage> notifyUserMessages = messageService.findMessageByUserId(userId, pageNo, pageSize);
        List<Message> messages = messageService.findMessageByNotifyMessages(notifyUserMessages);
        
        json.put("datas", JSONArray.fromObject(getMessageForUserList(notifyUserMessages,messages)));
        
        int count = messageService.getNotifyUserMessageCountByUserId(userId);
    ...
    private List<MessageForUserEntry> getMessageForUserList(List<NotifyUserMessage> notifyUserMessages,
