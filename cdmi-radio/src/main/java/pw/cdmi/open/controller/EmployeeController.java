...
public class EmployeeController {
        ...
        try {
            ...
            if (null != emp) {
                if (null != emp.getAccountId()) {
                    ...
                    SiteUser siteUser = userService.getSiteUserByAccountId(emp.getAccountId());
                    if (null != userAccount) {
    ...
    public boolean findEmployeeIdCard(@RequestParam(value = "employeId", defaultValue = "0") String employeId,
        ...
        if (certificateType1.equals(certificateType) || certificateType2.equals(certificateType)
            ...
            people = userService.getPeopleByIdCode(certificateNumber);
            if (people == null) {
                people = userService.getPeopleByDriverLicenseNumber(certificateNumber);
                if (people == null) {
                    people = userService.getPeopleBySocialCode(certificateNumber);
                }
            }
        } else if (certificateType4.equals(certificateType)) {
            people = userService.getPeopleByPassportNumber(certificateNumber);
        }
