...
//		}
		...
		for (MethodParameter mp : parameters) {
			...
			if (dataAuth != null) {
				...
				Object condition = this.arguments[i];
				//FIXME 目前只支持基本类型的查询
				logger.info("输出的" + condition);
				String sql = "select roleEnum from SiteDataPrivilegeAcl where authEnum = '" + condition + "' and appId ='"
						+ appService.getCurrentAppId() + "'";
				List<String> roles = daoImpl.find(sql);
