...
/**
 * 如果当前用户存在访问角色，则加载资源与权限的对应关系
 * 如果验证成功，转到该类执行获取所有的资源及其对应角色的关联关系 前置方法是FilterSecurityInterceptor的doFilter方法
...
//	private List<String> urls;
	...
	public Collection<ConfigAttribute> getAttributes(Object object)
		...
		// 获取所有角色。
		String sql = "select r from SiteRole r  where r.siteId = '" 
				+ siteService.getCurrentAppId() + "'";
		if(StringUtils.isEmpty(loginRole)){
			sql = "select r from SiteRole r where r.siteId = '" 
					+ siteService.getCurrentAppId() + "' and r.loginRole＝'"
					+ loginRole + "'";
		}
		List<SiteRole> roleList = daoImpl.find(sql);
		...
		// 获取所有资源
		List<SiteMenu> menulist = daoImpl.find("from SiteMenu where siteId='" + siteService.getCurrentAppId()  + "'");
		...
		//获取权限和资源的对应关系
		List<SiteMenuAcl> acllist = daoImpl
				.find("from SiteMenuAcl where siteId='" + siteService.getCurrentAppId()  + "'");
		...
		for (SiteRole role : roleList) {
			...
			for (SiteMenu menu : menulist) {
				for (SiteMenuAcl acl : acllist) {
					if (acl.getMenuId() == menu.getId()
							&& acl.getRoleId() == role.getId()) {
						String url = menu.getResourceURL();
