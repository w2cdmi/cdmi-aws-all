...
export default class MoveAndCopyModal extends Component {
  constructor (props) {
    ...
    this.state = {
      ...
      selectedKeys: ['0'],
      treeData: [
        { name: this.props.spaceName, id: '0' },
      ],
    }
  }
  ...
  onLoadData = (treeNode) => {
    return new Promise((resolve) => {
      ...
      this.ds.url = `/folders/${this.props.teamSpaceId}/${folderId}/items`
      this.ds.post((data) => {
  ...
  onCopyClick = () => {
    if (this.props.onTeamCopyClick) {
      const folderId = this.currentSelectedNode.props.dataRef.id
      this.props.onTeamCopyClick(parseInt(folderId))
    }
  }

  onMoveClick = () => {
    if (this.props.onTeamMoveClick) {
      const folderId = this.currentSelectedNode.props.dataRef.id
      this.props.onTeamMoveClick(parseInt(folderId))
    }
  }
