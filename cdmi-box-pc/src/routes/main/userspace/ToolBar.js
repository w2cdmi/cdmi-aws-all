...
export default class ToolBar extends React.Component {
  ...
  render () {
    ...
    const menu = (
      <Menu>
        <Menu.Item>
          <span onClick={() => this.props.saveToTeamSpaceRecord(selectedRecords)} style={{ width: '100%', display: 'inline-block' }}>转发到团队空间</span>
        </Menu.Item>
        <Menu.Item>
          <span onClick={() => this.props.moveAndCopyRecord(selectedRecords)} style={{ width: '100%', display: 'inline-block' }}>复制/移动</span>
        </Menu.Item>
        <Menu.Item>
