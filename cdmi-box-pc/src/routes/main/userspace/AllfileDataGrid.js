      ...
      render: (text, record) => {
        const menu = (
          <Menu>
            ...
            {!record.extraType &&
              <Menu.Item>
                <span onClick={() => { this.props.saveToTeamSpaceRecord(record) }} style={{ width: '100%', display: 'inline-block' }}>转发到团队空间</span>
              </Menu.Item>
            }
            {!record.extraType &&
              <Menu.Item>
                <span onClick={() => { this.props.moveAndCopyRecord(record) }} style={{ width: '100%', display: 'inline-block' }}>复制/移动</span>
              </Menu.Item>
            }
            {!record.extraType &&
              <Menu.Item>
