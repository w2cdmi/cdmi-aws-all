...
Page({
    ...
    onShow: function () {
        ...
        // } else if (crumb.name != "搜索结果" && page.data.isHide) {
        ...
        // } else {
        //     if (page.data.searchKeyWord !== undefined && page.data.searchKeyWord != "") {
        //         page.keepSearchPage();
        //     } else {
        //         page.lsOfFolderForClickCrum(app.globalData.token, crumb.ownerId, crumb.nodeId);
        //     }
    ...
    onShareAppMessage: function (e) { //打开手机右上角的转发事件
        ...
        if (e.from == "button") {
            ...
            url = '/disk/index?ownerId=' + ownerId + "&nodeId=" + nodeId + "&enterpriseId=" + getApp().globalData.enterpriseId;
    ...
    onFolderItemClick: function (e) {
        ...
        wx.navigateTo({
            url: '/disk/widget/filelist?ownerId=' + getApp().globalData.cloudUserId + '&nodeId=' + folderNodeId + '&name=' + encodeURIComponent("个人文件") + '&folderNodeName=' + folderNodeName,
        });
    },
    // 页面卸载
    onUnload: function () {
        ...
        wx.getStorage({
            ...
            success: (res) => {
                wx.setStorage({
                    key: 'fileListCrumbs',
                    data: res.data.splice(0, res.data.length - 1),
                })
            },
        })
        ...
        File.lsOfFolder(app.globalData.token, crumbs[0].ownerId, crumbs[crumbs.length - 1].nodeId, function (data) {
            ...
            if (dataFolders.length > 0 || dataFiles.length > 0) {
                ...
                console.log(
                    ...
                    data.totalCount +
                    " - acttotal:" +
                    (newFolders.length + newFiles.length)
                );
