...
Page({
    ...
    onLoad: function (options) {
        ...
        if (options.hasOwnProperty('type') && options.type == 'code') {
            wx.redirectTo({
                url: '/disk/batchShare/extractionEode/extractionEode?type=' + options.type + "&forWardId=" + options.forWardId + "&linkCode=" + options.linkCode + "&plainAccessCode=" + options.plainAccessCode,
            })
            this.setData({
                params: options
            })
    ...
    onShow: function () {
        ...
        //外发分享文件，带有企业id，则直接根据企业id登录，没有则不传企业id登录
    ...
    onShareAppMessage: function (options) {
        return {
            title: linkName,
            path: '/disk/index?linkCode=' + linkCode + '&forwardId=' + getApp().globalData.cloudUserId,
            imageUrl: shareIcon,
    ...
    onNodeItemClick: function (e) {
        ...
        if (fileInfo.type < 1) {
            wx.navigateTo({
                url: '/disk/shares/sharefolder?ownerId=' + fileInfo.ownedBy + "&nodeId=" + fileInfo.id + "&linkCode=" + linkCode
            })
            return;
        }
        ...
        if (ext == 'mp3') {
            ...
            file.openFile(fileInfo, null, "link," + linkCode);
        }
    },
...
function getLinkInfo(linkCode, page) {
    linkClient.getLinkInfo(linkCode, function (data) {
        ...
        if (data.subFileList.length == 0) {
            ...
            if (node.type < 1) {
                node = file.convertFolderList(data.subFileList)[0];
            } else {
                var files = file.convertFileList(data.subFileList);
                node = files[0];
            ...
            data.subFileList.map((item, index) => {
                if (item.type < 1) {
                    item = file.convertFolderList([item])[0];
                } else {
                    tempFiles.push(item);
                    item = file.convertFileList([item])[0];
                }
            });
            wx.setStorageSync('isPagination', false);   //不是分页
            var files = file.convertFileList(tempFiles);   //生成预览图片地址
            page.data.previewImageUrls = files.previewImageUrls;
            data.subFileList = tempFiles;
            //换成大图标
            data.subFileList.map((item, index) => {
                if (item.thumbnailUrlList.length > 0) {
                    var thumbnailUrl = item.thumbnailUrlList[0].thumbnailUrl;
                    var arr = [];
                    arr = thumbnailUrl.split("?");
                    arr[1] = "minHeight=200&minWidth=200";
                    data.subFileList[index].icon = arr[0] + "?" + arr[1];
                }
            })
            page.setData({
