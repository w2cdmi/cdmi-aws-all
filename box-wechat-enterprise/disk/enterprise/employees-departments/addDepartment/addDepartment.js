...
Page({
    ...
    toRoleSet: function() {
        ...
        if (page.data.selectedDepartment !== -1) {
            // 设置用户部门
            enterpriseClient.addDepartmentEmployment(page.data.deptId, page.data.employeeId, function (res) {
                var res = res;
                wx.navigateTo({
    ...
    onLoad: function (options) {
        enterpriseClient.getDepartment(options.employeeId, function (data) {
            var data = data;
        });
        this.setData({
