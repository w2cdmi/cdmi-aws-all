...
// 退出协作空间
function quitTeam(taemId, userId, callback) {
    var header = {
        Authorization: getApp().globalData.token,
    };
    var params = {};
    return httpclient.remove(config.host + '/ufm/api/v2/teamspaces/' + taemId + '/memberships/user/' + userId, params, header, callback);
}
...
function previewImage(ownerId, nodeId, name) {
    ...
    if (imgUrls == undefined || imgUrls.length == 0) {
        ...
        for (var i = 0; i < imgUrls.length; i++) {
            if (imgUrls[i].indexOf(encodeURI(name)) != -1) {
                url = imgUrls[i];
                break;
            }
        }
...
function getPreUploadFileUrl(parentFileInfo, name, size, callback) {
    ...
    if (linkCode) {
        ...
        data = {
            ...
            size: size,
            parent: parentId,
            createdBy: getApp().globalData.userId
        }
    } 
...
function convertFileList(datas, isMultiselect) {
    var files = [];
...
function convertFolderList(data, isMultiselect) {
    var folders = [];
    for (var i = 0; i < data.length; i++) {
        var folder = data[i];
        if (typeof (folder.isListAcl) == 'undefined' || folder.isListAcl) {
            folder.icon = '/disk/images/icon/folder-icon.png';
        } else {
            folder.icon = '/disk/images/icon/folder-forbidden-icon.png';
        }
...
// 设为快捷目录
function setShortcut(params, belongType, callback) {
    var header = {
        Authorization: getApp().globalData.token
    };
